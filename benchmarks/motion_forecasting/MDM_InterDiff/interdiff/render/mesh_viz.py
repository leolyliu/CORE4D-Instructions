...
def visualize_body_obj_hho(body1_verts, body2_verts, body_face, obj_verts, obj_face, past_len=0, pcd=None, pcd_contact=None, multi_col=None, text="",
                       multi_angle=True, h=512, w=512, bg_color='white',
                       save_path=None, fig_label=None, use_hydra_path=True, sample_rate=1):
    """[summary]
    ...
    mesh1_rec = -body1_verts
    mesh2_rec = -body2_verts
    obj_mesh_rec = -obj_verts
    
    # align hho coordinate system to behave
    mesh1_rec[:, :, 0] = -mesh1_rec[:, :, 0]
    mesh1_rec[:, :, 1] = -mesh1_rec[:, :, 1]
    mesh2_rec[:, :, 0] = -mesh2_rec[:, :, 0]
    mesh2_rec[:, :, 1] = -mesh2_rec[:, :, 1]
    obj_mesh_rec[:, :, 0] = -obj_mesh_rec[:, :, 0]
    obj_mesh_rec[:, :, 1] = -obj_mesh_rec[:, :, 1]
    
    body_face = body_face[:, ::-1]
    obj_face = obj_face[:, ::-1]
    
    minx, _, miny = mesh1_rec.min(axis=(0, 1))
    maxx, _, maxy = mesh1_rec.max(axis=(0, 1))
    minsxy = (minx, maxx, miny, maxy)
    height_offset = np.min(mesh1_rec[:, :, 1])  # Min height

    mesh1_rec = mesh1_rec.copy()
    mesh1_rec[:, :, 1] -= height_offset
    mesh2_rec = mesh2_rec.copy()
    mesh2_rec[:, :, 1] -= height_offset
    obj_mesh_rec[:, :, 1] -= height_offset
    mesh1_rec[:, :, 0] -= (minx + maxx) / 2
    mesh2_rec[:, :, 0] -= (minx + maxx) / 2
    obj_mesh_rec[:, :, 0] -= (minx + maxx) / 2
    mesh1_rec[:, :, 2] -= (miny + maxy) / 2
    mesh2_rec[:, :, 2] -= (miny + maxy) / 2
    obj_mesh_rec[:, :, 2] -= (miny + maxy) / 2
    if vis_mar:
        pcd[:, :, 1] -= height_offset
    ...
    for i in range(seqlen):
        ...
        if i <= past_len:
            mesh1_color = np.tile(c2rgba(colors['light_grey']), (mesh1_rec.shape[1], 1))
            mesh2_color = np.tile(c2rgba(colors['light_grey']), (mesh2_rec.shape[1], 1))
        else:
            mesh1_color = np.tile(c2rgba(colors['yellow_pale']), (mesh1_rec.shape[1], 1))
            mesh2_color = np.tile(c2rgba(colors['cyan']), (mesh2_rec.shape[1], 1))
            
        m1_rec = trimesh.Trimesh(vertices=mesh1_rec[i], faces=body_face, vertex_colors=mesh1_color)
        m2_rec = trimesh.Trimesh(vertices=mesh2_rec[i], faces=body_face, vertex_colors=mesh2_color)
        all_meshes = []
        ...
        all_meshes = all_meshes + [obj_m_rec, m1_rec, m2_rec]
        mv.set_meshes(all_meshes, group_name='static')
        video_i = mv.render()
            ...
            for _ in range(3):
                ...
                if vis_mar:
                    ...
                    for bp, m_bp in pcd_bodyparts.items():
                        all_meshes.append(m_bp)
                obj_m_rec.apply_transform(Ry)
                m1_rec.apply_transform(Ry)
                m2_rec.apply_transform(Ry)
                all_meshes = all_meshes + [obj_m_rec, m1_rec, m2_rec]
                mv.set_meshes(all_meshes, group_name='static')
                video_views.append(mv.render())
            video_i = np.concatenate((np.concatenate((video_views[0], video_views[1]), axis=1), np.concatenate((video_views[3], video_views[2]), axis=1)), axis=1)
    ...
    if save_path is not None:
        imageio.mimsave(save_path, list(np.squeeze(video).astype(np.uint8)), fps=15 // sample_rate)
