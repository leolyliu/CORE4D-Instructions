...
class MDM(nn.Module):
    def __init__(self, args):
        ...
        self.bodyEmbedding1 = nn.Linear(args.smpl_dim+3, num_channels)
        self.bodyEmbedding2 = nn.Linear(args.smpl_dim+3, num_channels)
        ...
        self.bodyFinalLinear1 = nn.Linear(num_channels, args.smpl_dim+3)
        self.bodyFinalLinear2 = nn.Linear(num_channels, args.smpl_dim+3)
    ...
    def _get_embeddings(self, data, device=None):
        if device:
            body1_pose = torch.cat([frame['person1_params']['pose'][:, :66].unsqueeze(0) for frame in data['frames']], dim=0).float().to(device) # TxBxDb
            body1_trans = torch.cat([frame['person1_params']['trans'].unsqueeze(0) for frame in data['frames']], dim=0).float().to(device)  # TxBx3
            body2_pose = torch.cat([frame['person2_params']['pose'][:, :66].unsqueeze(0) for frame in data['frames']], dim=0).float().to(device) # TxBxDb
            body2_trans = torch.cat([frame['person2_params']['trans'].unsqueeze(0) for frame in data['frames']], dim=0).float().to(device)  # TxBx3
            ...
            body1_pose = torch.cat([frame['person1_params']['pose'][:, :66].unsqueeze(0) for frame in data['frames']], dim=0).float() # TxBxDb
            body1_trans = torch.cat([frame['person1_params']['trans'].unsqueeze(0) for frame in data['frames']], dim=0).float() # TxBx3
            body2_pose = torch.cat([frame['person2_params']['pose'][:, :66].unsqueeze(0) for frame in data['frames']], dim=0).float() # TxBxDb
            body2_trans = torch.cat([frame['person2_params']['trans'].unsqueeze(0) for frame in data['frames']], dim=0).float() # TxBx3
        ...
        body1_pose = matrix_to_rotation_6d(axis_angle_to_matrix(body1_pose.view(T, B, -1, 3))).view(T, B, -1)  # (T, B, 22*6)
        body2_pose = matrix_to_rotation_6d(axis_angle_to_matrix(body2_pose.view(T, B, -1, 3))).view(T, B, -1)  # (T, B, 22*6)
        obj_angles = matrix_to_rotation_6d(axis_angle_to_matrix(obj_angles.view(T, B, -1, 3))).view(T, B, -1)  # (T, B, 6)
        gt = torch.cat([body1_pose, body1_trans, body2_pose, body2_trans, obj_angles, obj_trans], dim=2)
        body1, body2, obj = torch.cat([body1_pose, body1_trans], dim=2), torch.cat([body2_pose, body2_trans], dim=2), torch.cat([obj_angles, obj_trans], dim=2)

        body1 = self.bodyEmbedding1(body1[:self.args.past_len])
        body2 = self.bodyEmbedding2(body2[:self.args.past_len])
        obj = self.objEmbedding(obj[:self.args.past_len])
        embedding = body1 + body2 + obj + pc_embedding
    ...
    def _decode(self, x, time_embedding, y=None):
        body1, body2, obj = torch.split(x, [self.args.smpl_dim+3, self.args.smpl_dim+3, 9], dim=2)
        body1 = self.bodyEmbedding1(body1)
        body2 = self.bodyEmbedding2(body2)
        obj = self.objEmbedding(obj)
        decoder_input = body1 + body2 + obj + time_embedding
        decoder_input = self.PositionalEmbedding(decoder_input)
        decoder_output = self.decoder(tgt=decoder_input, memory=y)

        body1 = self.bodyFinalLinear1(decoder_output)
        body2 = self.bodyFinalLinear2(decoder_output)
        obj = self.objFinalLinear(decoder_output)
        pred = torch.cat([body1, body2, obj], dim=2)
        return pred
