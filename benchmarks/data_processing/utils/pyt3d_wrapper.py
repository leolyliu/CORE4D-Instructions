...
# SMPL_OBJ_COLOR_LIST = [
#     [0.65098039, 0.74117647, 0.85882353],  # person1
#     [251 / 255.0, 128 / 255.0, 114 / 255.0],  # person2
#     [128 / 255.0, 251 / 255.0, 50 / 255.0],  # object
# ]
...
class MeshRendererWrapper:
    ...
    def __init__(self, image_size=(1200, 900),
        ...
        self.device = device
        self.lights=lights if lights is not None else AmbientLights(  # TODO
...
class Pyt3DWrapper:
    def __init__(self, image_size, device='cuda:0', colors=SMPL_OBJ_COLOR_LIST, use_fixed_cameras=False, eyes=None, intrin=None, extrin=None, lights=None):
        if lights is None:
            lights=PointLights(device=device, location=[[0.0, 2, 0.0]])  # TODO: 尝试改成[0,2,0]
    ...
    def get_surround_cameras(self, intrin, image_size, radius=3.0, eyes=None, n_poses=20, up=(0.0, 1.0, 0.0), device='cuda:0'):
        ...
        for eye in eyes:
            ...
            # self.extrin = torch.from_numpy(self.extrin).type(torch.float64).to(device)
            ...
            self.intrin[1, 2] = cy
            self.intrin[2, 2] = 1.0
            # self.intrin = torch.from_numpy(self.intrin).type(torch.float64).to(device)
            pyt3d_version = pytorch3d.__version__
            if pyt3d_version >= '0.6.0':
