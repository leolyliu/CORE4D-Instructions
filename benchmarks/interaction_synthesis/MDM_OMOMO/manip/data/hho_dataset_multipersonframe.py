...
class HHODataset(Dataset):
    def __init__(
        ...
        if False:
            ...
            self.data_dict = self.load_data(data_root_folder, date_list=date_list)
    ...
    def load_data(self, data_root_folder, date_list):
        """
        ...
        for date in date_list:
            ...
            for seq_name in os.listdir(date_dir):
                ...
                # prepared_data["obj_verts"] = ((prepared_data["obj_rot"] @ data["object_points"][:, :3].T) + prepared_data["obj_trans"]).transpose(0, 2, 1)  # (T, N_point, 3)
    ...
    def __getitem__(self, index):  # TODO: modify
        ...
        for person in ["person1", "person2"]:
            ...
            data_input_dict[person] = {}
            data_input_dict[person]['motion'] = paded_new_data_input  # normalized joint positions + global rotations, shape = (T, 24*3+22*6)
            data_input_dict[person]['ori_motion'] = paded_ori_data_input  # original joint positions + global rotations, shape = (T, 24*3+22*6)
            data_input_dict[person]['obj_com_pos'] = paded_obj_com_pos  # (T, 3)
            data_input_dict[person]['obj_rot_mat'] = paded_obj_rot_mat  # (T, 3, 3)
            data_input_dict[person]['obj_scale'] = paded_obj_scale  # (T,)
            data_input_dict[person]['obj_trans'] = paded_obj_trans  # (T, 3)
            data_input_dict[person]['betas'] = self.window_data_dict[index][person]['betas']  # (1, 10)
            data_input_dict[person]['gender'] = str(self.window_data_dict[index][person]['gender'])  # "neutral"
            data_input_dict[person]['trans2joint'] = trans2joint  # 这个beta在SMPLX TPOSE下的pelvis坐标的负值, (3,)
            
            assert data_input_dict[person]['motion'].shape == (self.window, 24*3+22*6)
            assert data_input_dict[person]['ori_motion'].shape == (self.window, 24*3+22*6)
            assert data_input_dict[person]['obj_com_pos'].shape == (self.window, 3)
            assert data_input_dict[person]['obj_rot_mat'].shape == (self.window, 3, 3)
            assert data_input_dict[person]['obj_scale'].shape == (self.window,)
            assert data_input_dict[person]['obj_trans'].shape == (self.window, 3)
            
            # # data check
            # pcd = o3d.geometry.PointCloud()
            # pcd.points = o3d.utility.Vector3dVector(paded_ori_data_input[110, :24*3].reshape(24, 3))
            # o3d.io.write_point_cloud(person + "_ori_motion.ply", pcd)
            # print("[__getitem__]", person, data_input[110, :72].reshape(24, 3))
            # print(paded_obj_com_pos[110], paded_obj_trans[110])
