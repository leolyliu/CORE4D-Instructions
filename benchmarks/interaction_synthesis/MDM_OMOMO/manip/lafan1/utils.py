...
def rotate_at_frame_w_obj_specify_frame(X, Q, X_frame, Q_frame, obj_x, obj_q, trans2joint_list, parents, n_past=1, floor_z=False):
    """
    X_frame, Q_frame: 用来指定对齐到的坐标系的human的信息, 对齐到它的第n_past-1帧
    """
    
    
    # Get global quats and global poses (FK)
    global_q, global_x = quat_fk(Q, X, parents)
    global_q_frame, global_x_frame = quat_fk(Q_frame, X_frame, parents)

    key_glob_Q = global_q_frame[:, n_past - 1 : n_past, 0:1, :]  # (B, 1, 1, 4)
    if floor_z: 
        # The floor is on z = xxx. Project the forward direction to xy plane. 
        forward = np.array([1, 1, 0])[np.newaxis, np.newaxis, np.newaxis, :] * quat_mul_vec(
            key_glob_Q, np.array([1, 0, 0])[np.newaxis, np.newaxis, np.newaxis, :]
